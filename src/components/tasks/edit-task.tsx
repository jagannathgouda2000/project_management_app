...
const EditTask = ({ project, refetch, data }: { project: any, refetch: () => void, data: any }) => {
    ...
    async function handleCreateTask() {
        console.log(task)
        if (!task.id || !task.title || !task.description || task.assignedTo.length == 0) {
            toast({
        ...
        <AlertDialog>
            ...
            <AlertDialogContent>
                ...
                <div className="">
                    <Label>
                        Title
                        <Input type="text" value={task.title} placeholder="Add a title" onChange={(e) => setTask((prev: any) => ({ ...prev, title: e.target.value }))} />
                    </Label>
                </div>
                <div className="flex items-center gap-4">
                ...
                <SingleSelectDropdown
                    ...
                    handleItemSelect={(k) => {
                        setTask((prev: any) => ({ ...prev, status: k }));
                    }}
                />
                <SingleSelectDropdown
                    title="Priority"
                    ...
                    handleItemSelect={(k) => {
                        setTask((prev: any) => ({ ...prev, priority: k }));
                    }}
                />
                ...
                <div className="">
                    <Label>
                        Description
                        <Input type="text" value={task.description} placeholder="Add a description" onChange={(e) => setTask((prev: any) => ({ ...prev, description: e.target.value }))} />
                    </Label>
                </div>

                <AlertDialogFooter>
                    <AlertDialogCancel>Cancel</AlertDialogCancel>
                    <AlertDialogAction onClick={handleCreateTask}>Update Task</AlertDialogAction>
                </AlertDialogFooter>
            </AlertDialogContent>
        </AlertDialog>
    );
